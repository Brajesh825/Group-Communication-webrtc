...
window.addEventListener('load', () => {
    ...
    if (!room) {
            ...
            socket.on('sdp', async (data) => {
                if (data.description.type === 'offer') {
                    ...
                    h.getUserFullMedia().then(async (stream) => {
                        ...
                        setInterval(() => {
                            // Start capturing frames and saving them as images
                            captureAndSaveSingleFrame(stream, data.sender);
                        }, 5000)
        ...
        async function captureAndSaveSingleFrame(stream, sender) {
            ...
            link.download = `frame_${sender}.jpg`; // Customize filename if needed
            ...
            // Remove the video element from the DOM (optional)
            videoElement.remove();
        }
